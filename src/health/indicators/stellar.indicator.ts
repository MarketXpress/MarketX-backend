...
export class StellarIndicator {
  private readonly stellarHorizonUrl =
  ...
  async isHealthy(): Promise<HealthIndicatorResult> {
    try {
      ...
      const response = (await axios.get(
        `${this.stellarHorizonUrl}/ledgers?limit=1`,
        {
          timeout,
        },
      )) as any;
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        if (
          ...
          return {
            stellar: {
              ...
              network: this.stellarHorizonUrl.includes('testnet')
                ? 'testnet'
                : 'mainnet',
              last_ledger:
          ...
          throw new HealthCheckError(
            'Stellar API returned invalid response structure',
            {
              stellar: {
                status: 'down',
        ...
        throw new HealthCheckError(
          'Stellar Horizon API returned non-success status',
          {
            stellar: {
              status: 'down',
      ...
      throw new HealthCheckError('Stellar network check failed', {
        stellar: {
          ...
          message:
            error.message ||
            'Unknown error occurred while checking Stellar network',
        },
      });
    }
  }
