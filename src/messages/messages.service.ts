...
export class MessagesService {
  ...
  async sendMessage(sendMessageDto: SendMessageDto, senderId: number) {
    ...
    if (!isOrderParticipant) {
      throw new ForbiddenException('Only order participants can send messages');
    }
  ...
  async getMessagesByOrderId(orderId: number, userId: number) {
    ...
    if (!isOrderParticipant) {
      throw new ForbiddenException('Only order participants can view messages');
    }
  ...
  private async checkIfOrderParticipant(
    orderId: number,
    userId: number,
  ): Promise<boolean> {
  ...
  private emitNotificationEvent(message: Message) {
    ...
    this.logger.log(
      `New message notification: Message ${message.id} sent to user ${message.receiverId} for order ${message.orderId}`,
    );
