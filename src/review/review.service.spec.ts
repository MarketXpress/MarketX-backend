 ...
 */
function buildMockEntityManager(overrides: Partial<EntityManager> = {}): Partial<EntityManager> {
  return {
    query: jest.fn().mockImplementation((sql: string) => {
      if (sql.startsWith('SELECT id')) return Promise.resolve([{ id: PRODUCT_ID, averageRating: 4, reviewCount: 1 }]);
      if (sql.startsWith('SELECT o.id')) return Promise.resolve([{ id: 'order-id' }]);
...
describe('ReviewsService', () => {
  ...
  describe('create()', () => {
    it('should create and return a review on the happy path', async () => {
      ...
      mockDataSource.transaction.mockImplementation((cb: (em: EntityManager) => Promise<Review>) =>
    ...
    it('should set body to null when not provided', async () => {
      ...
      expect(em.create).toHaveBeenCalledWith(Review, expect.objectContaining({ body: null }));
    });
    ...
    it('should throw NotFoundException when product does not exist', async () => {
      ...
      await expect(service.create(PRODUCT_ID, USER_ID, makeDto())).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw ForbiddenException when buyer has no COMPLETED order', async () => {
      const em = buildMockEntityManager({
        query: jest.fn().mockImplementation((sql: string) => {
          if (sql.startsWith('SELECT id')) return Promise.resolve([{ id: PRODUCT_ID }]);
