...
export class RateLimitConfigController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieve rate limiting analytics data for monitoring and analysis',
  })
  @ApiQuery({
    name: 'days',
    required: false,
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        message: {
          type: 'string',
          example: 'Tier configuration updated successfully',
        },
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid endpoint or configuration',
  })
  @HttpCode(HttpStatus.OK)
  ...
  @ApiOperation({
    ...
    description:
      'Reset the rate limit counter for a specific identifier and optionally endpoint',
  })
  @ApiBody({
  ...
  @ApiOperation({
    ...
    description:
      'Check if the rate limiting service and Redis connection are healthy',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 503,
    description: 'Rate limiting service is unhealthy',
  })
  async checkHealth() {
    // This would need to be implemented in the service
    return {
