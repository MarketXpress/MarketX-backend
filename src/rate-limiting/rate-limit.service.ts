...
export class RateLimitService {
  ...
  private initializeRedis() {
    ...
    if (typeof RedisLib === 'function') {
      ...
      try {
        ...
        this.logger.warn(
          'Unable to initialize Redis client; operating in memory-only mode',
        );
        this.redis = {} as any;
      }
    }
  ...
  private generateHeaders(
    ...
    return {
      ...
      'Retry-After':
        msBeforeNext > 0 ? Math.ceil(msBeforeNext / 1000).toString() : '0',
    };
  }
  ...
  async getAnalytics(
    days: number = 7,
  ): Promise<Array<{ date: string; data: any[] }>> {
    try {
  ...
  async updateEndpointConfig(
    ...
    await this.redis.set(
      configKey,
      JSON.stringify(this.endpointConfigs[endpoint]),
    );
  }
  ...
  async loadConfigurations(): Promise<void> {
    try {
      ...
      const endpointKeys = await this.redis.keys(
      ...
      this.logger.warn(
        'Failed to load rate limit configurations from Redis:',
        error,
      );
    }
  }
